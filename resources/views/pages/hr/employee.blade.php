...
<div class="w-full space-y-6" x-data="{
    ...
    isSubmitting: false,
    form: {
        id: null, nik: '', first_name: '', last_name: '', gender: '',
        position: '', division: '', partnership_id: '',
    ...
    openCreate() {
        ...
        this.errors = {};
        this.form = { id: null, nik: '', first_name: '', last_name: '', gender: '',
                    position: '', division: '', partnership_id: '', manager_id: '', date_of_entry: '' };
        this.showModal = true;
    },
    ...
    <div x-show="showResignModal" x-cloak
        ...
        <div class="relative w-full max-w-sm mx-4 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden" @click.stop>
            <!-- Header -->
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/[0.05]">
            ...
            <div class="px-6 py-5 space-y-4">
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    Tandai <span class="font-semibold text-gray-800 dark:text-white" x-text="resignForm.name"></span> sebagai karyawan resign.
        ...
        <div class="p-6 border-b border-gray-100 dark:border-white/[0.05]">
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <form action="{{ route('employees.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
                    ...
                    <select name="position" onchange="this.form.submit()" class="px-3 py-2 border border-gray-200 rounded-lg text-sm dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 border-gray-300 bg-transparent text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    ...
                    <select name="partnership" onchange="this.form.submit()" class="px-3 py-2 border border-gray-200 rounded-lg text-sm dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 border-gray-300 bg-transparent text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    ...
                    <select name="division" onchange="this.form.submit()" class="px-3 py-2 border border-gray-200 rounded-lg text-sm dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 border-gray-300 bg-transparent text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                ...
                <button @click="openCreate()" class="inline-flex justify-center items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors">
                ...
                <table class="w-full">
                        ...
                        <tr>
                            <th class="px-6 py-3 text-start">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'nik', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc', 'tab' => 'active']) }}"
                                    ...
                                    NIK <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" stroke-width="2"/></svg>
                                </a>
                            </th>
                            <th class="px-6 py-3 text-start">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'first_name', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc', 'tab' => 'active']) }}"
                                    class="flex items-center gap-1 text-theme-xs font-medium text-gray-500 hover:text-blue-600">
                                    Name <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" stroke-width="2"/></svg>
                                </a>
                            </th>
                            ...
                            <th class="px-6 py-3 text-start">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'partnership', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc', 'tab' => 'active']) }}"
                                    class="flex items-center gap-1 text-theme-xs font-medium text-gray-500 hover:text-blue-600">
                                    Teams <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" stroke-width="2"/></svg>
                                </a>
                            </th>
                            ...
                            <th class="px-6 py-3 text-start">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'date_of_entry', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc', 'tab' => 'active']) }}"
                                    class="flex items-center gap-1 text-theme-xs font-medium text-gray-500 hover:text-blue-600">
                                    Entry Date <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" stroke-width="2"/></svg>
                                </a>
                            </th>
                    ...
                    <tbody>
                        ...
                        <template x-if="activeData.length === 0">
                            <tr>
                                <td colspan="7" class="px-6 py-10 text-center text-sm text-gray-400">No active employees found.</td>
                            </tr>
                        </template>
                    </tbody>
                </table>
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-3 text-start">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'release_date', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc', 'tab' => 'resigned']) }}"
                                    class="flex items-center gap-1 text-theme-xs font-medium text-gray-500 hover:text-blue-600">
                                    Resign Date <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" stroke-width="2"/></svg>
                                </a>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <template x-for="row in resignedData" :key="row.id">
                            <tr class="border-b border-gray-100 dark:border-white/[0.05] hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                                ...
                                <td class="px-6 py-3.5">
                                    <div class="flex items-center gap-3">
                                        <div class="flex h-10 w-10 items-center justify-center rounded-full text-sm font-medium bg-gray-100 text-gray-400">
                                            <span x-text="row.initials"></span>
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-500" x-text="row.division"></td>
                                <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-500" x-text="row.partnership"></td>
                                <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-500" x-text="row.position"></td>
                                <td class="px-6 py-3.5">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-50 text-red-600" x-text="row.release_date"></span>
                                </td>
                            </tr>
                        </template>
                        <template x-if="resignedData.length === 0">
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center text-sm text-gray-400">No resigned employees found.</td>
                            </tr>
                        </template>
                    </tbody>
                </table>
