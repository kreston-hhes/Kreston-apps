...
@section('content')
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <form action="{{ route('employees.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
                            <div class="relative w-full sm:w-64">
                                ...
                                <input type="search" @keydown.enter="$el.form.submit()" name="search"
                                    value="{{ request('search') }}" placeholder="Search NIK or Name..."
                            ...
                            <x-fillter.select name="position" onchange="this.form.submit()"
                                class="px-3 py-2 border border-gray-200 rounded-lg text-sm dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 border-gray-300 bg-transparent bg-none text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            ...
                            <x-fillter.select name="partnership" onchange="this.form.submit()"
                                class="px-3 py-2 border border-gray-200 rounded-lg text-sm dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 border-gray-300 bg-transparent bg-none text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">

                                <option value="">All Teams</option>

                                @foreach ($partnerships as $partnership)
                                    <option value="{{ $partnership }}" @selected(request('partnership') == $partnership)>
                                        {{ $partnership }}
                                    </option>
                                @endforeach
                            ...
                            <x-fillter.select name="division" onchange="this.form.submit()"
                                class="px-3 py-2 border border-gray-200 rounded-lg text-sm dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 border-gray-300 bg-transparent bg-none text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                                <option value="">All Divisions</option>
                                @foreach ($divisions as $div)
                                    <option value="{{ $div }}" @selected(request('division') == $div)>
                                        {{ $div }}
                                    </option>
                                @endforeach
                            </x-fillter.select>
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 dark:bg-gray-900">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-start">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'date_of_entry', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}"
                                    class="flex items-center gap-1 text-theme-xs font-medium text-gray-500 hover:text-blue-600">
                                    Entry Date
                                    <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" stroke-width="2" />
                    ...
                    <tbody>
                        <template x-for="row in tableRowData" :key="row.id">
                            <tr
                                ...
                                <td class="px-6 py-3.5 text-theme-sm text-gray-700 dark:text-gray-400" x-text="row.nik">
                                </td>
                                ...
                                <td class="px-6 py-3.5 text-theme-sm text-gray-700 dark:text-gray-400" x-text="row.manager">
                                </td>
                                <td class="px-6 py-3.5 text-theme-sm text-gray-700 dark:text-gray-400"
    ...
    <x-modal.modal name="add-employee" title="Add Employee">

        <form>

            <x-test.text-input name="name" label="Namepos" placeholder="Input name" />
