    ...
    public function submitTicket(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            if (!$ticket) {
                ...
                AlertService::notify('error', 'Failed', 'Failed to submit support ticket.');
            ...
            DB::afterCommit(function () use ($ticket) {
                $itEmails = Employee::where('division', 'IT')->where('status', 'active')->whereNotNull('email')->pluck('email')->filter()->unique()->values()->toArray();

                Mail::to($ticket->requester_email)->bcc($itEmails)->send(new TicketSubmittedMail($ticket));
            });
            AlertService::notify('success', 'Ticket Submitted', 'Your support ticket ' . $ticket->id_ticket . ' has been submitted successfully.');
            ...
            AlertService::notify('error', 'System Error', 'An error occurred while submitting the ticket.');
